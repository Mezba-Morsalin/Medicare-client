...
const DoctorPage = async () => {
  ...
  <p className="mt-4 max-w-2xl mx-auto text-slate-500">
    Search doctors by name or specialization and compare
    consultation fees, experience, and ratings to find the
    perfect healthcare professional.
  </p>
</div>
<DoctorFilters/>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {
                doctors.map(doctor => <AllDoctors key={doctor._id} doctor = {doctor}></AllDoctors>)
            }
        </div>
        </div>
    );
